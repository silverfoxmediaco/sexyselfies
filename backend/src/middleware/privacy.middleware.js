...
exports.checkMemberOptIn = async (req, res, next) => {
  try {
    ...
    if (interactionType && memberAnalytics.privacy.interactionPreferences) {
      const allowed = memberAnalytics.privacy.interactionPreferences[interactionType];
...
exports.checkBulkMessagePermissions = async (req, res, next) => {
  try {
    ...
    if (blockedRecipients.length > 0) {
      req.body.recipients = recipients.filter(id => 
        !blockedRecipients.includes(id)
      );
...
function applyVisibilityRules(data, rules) {
  ...
  if (typeof data === 'object') {
    ...
    for (const [key, value] of Object.entries(data)) {
      ...
      if (!rules.canSeeExactSpending && key.includes('spending') && typeof value === 'number') {
...
async function exportUserData(userId, userRole) {
  ...
  if (userRole === 'member') {
    ...
    const interactions = await MemberInteraction.find({ member: member._id })
      .limit(100);
    ...
    const salesActivity = await CreatorSalesActivity.findOne({ creator: creator._id });
