...
exports.verifiedCreatorOnly = async (req, res, next) => {
  try {
    ...
    const creator = await Creator.findById(creatorId).select(
      'isVerified verificationLevel verificationStatus'
    );
...
exports.verificationTierAccess = allowedTiers => {
  return async (req, res, next) => {
    try {
      ...
      const creator = await Creator.findById(creatorId).select(
        'isVerified verificationLevel subscriptionTier'
      );
...
exports.batchVerificationCheck = async (req, res, next) => {
  try {
    ...
    const creator = await Creator.findById(creatorId).select(
      'isVerified verificationLevel verificationStatus verificationDocuments'
    );
    ...
    req.verificationInfo = {
      ...
      canSendBulkMessages:
        creator.verificationLevel === 'premium' ||
        creator.verificationLevel === 'vip',
