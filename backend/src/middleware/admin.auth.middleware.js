...
exports.requireAllPermissions = (...permissions) => {
  return (req, res, next) => {
    ...
    const missingPermissions = permissions.filter(p => !req.admin.permissions[p]);
  ...
  return async (req, res, next) => {
    ...
    res.send = function(data) {
      ...
      if (res.statusCode < 400) {
        const targetUser = req.params.userId || req.params.id || req.body.userId;
        ...
        req.admin.logAction(action, targetUser, 'User', targetContent, reason)
...
exports.canModifyUser = async (req, res, next) => {
  try {
    ...
    if (req.admin.role === 'moderator' && req.admin.permissions.canSuspendUsers) {
      return next();
    }
