  ...
  try {
    ...
    switch (level) {
      case 'minimal':
        ...
        return {
          ...
          velocity: showTrend ? {
            ...
            direction: anonymized.velocity?.trend === 'increasing' ? 'up' : 
                      anonymized.velocity?.trend === 'decreasing' ? 'down' : 'stable'
          } : null,
      ...
      case 'standard':
        ...
        return {
          tier: showTier ? anonymized.tier : 'hidden',
          last30DaysRange: showRange ? getSpendingRange(anonymized.last30Days) : null,
          lifetimeRange: showRange ? getLifetimeRange(anonymized.lifetime) : null,
          velocity: showTrend ? {
            trend: anonymized.velocity?.trend,
            percentageRange: getPercentageRange(anonymized.velocity?.percentageChange)
          } : null,
      ...
      default:
        // Default to standard
        return exports.anonymizeSpendingData(spendingData, { ...options, level: 'standard' });
    }
  ...
  try {
    ...
    const processObject = (obj, path = '') => {
      ...
      for (const [key, value] of Object.entries(obj)) {
        ...
        if (shouldHideField(lowerKey, value, {
          hideEmail,
          hidePhone,
          hidePayment,
          hideLocation,
...
exports.applyPrivacySettings = async (data, memberId, context = {}) => {
  try {
    ...
    if (!privacy.discoverable && requesterRole !== 'admin' && purpose !== 'moderate') {
      return {
    ...
    switch (dataType) {
      case 'profile':
        filteredData = applyProfilePrivacy(filteredData, privacy, requesterRole);
        break;
        
      case 'analytics':
        filteredData = applyAnalyticsPrivacy(filteredData, privacy, requesterRole);
        break;
        
      case 'activity':
        filteredData = applyActivityPrivacy(filteredData, privacy, requesterRole);
        break;
        
      case 'interaction':
        filteredData = applyInteractionPrivacy(filteredData, privacy, requesterRole);
        break;
        
      default:
        // Apply general privacy
        filteredData = applyGeneralPrivacy(filteredData, privacy, requesterRole);
    }
 ...
 */
exports.checkDataPermissions = async (requesterId, targetId, accessRequest = {}) => {
  try {
    const {
    ...
    if (requesterRole === 'admin') {
      return {
        ...
        restrictions: ['Cannot edit personal data', 'Cannot delete without approval'],
    ...
    switch (dataType) {
      case 'profile':
        if (privacy.discoverable || requesterRelation === 'matched' || requesterRelation === 'customer') {
      ...
      case 'analytics':
        if (requesterRole === 'creator' && (requesterRelation === 'customer' || requesterRelation === 'matched')) {
          permission.allowed = true;
      ...
      case 'messages':
        if (requesterRelation === 'matched' || requesterRelation === 'customer') {
    ...
    if (permission.allowed && action !== 'view') {
      permission.actions = permission.actions.filter(a => {
        ...
        if (action === 'delete') return requesterRelation === 'self' || requesterRole === 'admin';
        if (action === 'export') return requesterRelation === 'self' || requesterRole === 'admin';
...
function maskField(fieldName, value, requesterRelation) {
  ...
  if (lowerField.includes('address')) {
    return typeof value === 'string' ? value.substring(0, 5) + '...' : '[Hidden]';
  }
  
  // ID masking (show partial)
  if (lowerField.includes('id') && typeof value === 'string' && value.length > 8) {
...
function applyProfilePrivacy(data, privacy, requesterRole) {
  ...
  if (filtered.activity && !privacy.discoverable) {
    filtered.activity = {
      ...
      isActive: filtered.activity.lastActive > new Date(Date.now() - 7 * 24 * 60 * 60 * 1000)
    };
  }
...
function applyAnalyticsPrivacy(data, privacy, requesterRole) {
  ...
  if (filtered.engagement && !privacy.showEngagement) {
    filtered.engagement = {
      ...
      activityLevel: filtered.engagement.messageResponseRate > 50 ? 'high' : 'normal'
    };
  }
 ...
 */
exports.filterMembersForPrivacy = async (members, requesterId, options = {}) => {
  const filtered = [];
