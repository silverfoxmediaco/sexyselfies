...
exports.segmentBySpending = async (options = {}) => {
  try {
    ...
    const members = await MemberAnalytics.find(query).populate(
      'member',
    ...
    for (const [segmentName, memberList] of Object.entries(segments)) {
      enhancedSegments[segmentName] = {
        ...
        compatibility: matchCreatorContent
          ? calculateCreatorCompatibility(memberList, creatorCategories)
          : null,
...
exports.createCustomSegments = async (criteria = {}) => {
  try {
    const { creatorId = null, segments = [] } = criteria;
    ...
    for (const segment of segmentsToProcess) {
      ...
      const members = await MemberAnalytics.find(query).populate(
        'member',
        'username avatar joinDate lastActive'
      );

      // Apply additional filtering for complex conditions
      const filteredMembers = await applyComplexFilters(
        members,
        segment.conditions
      );
...
function segmentByActivityPattern(members) {
  ...
  members.forEach(member => {
    ...
    if (
      member.activity.hourlyActivity &&
      member.activity.hourlyActivity.length > 0
    ) {
...
function segmentByCategories(members, creatorCategories = []) {
  ...
  members.forEach(member => {
    const memberCategories = member.preferences.topCategories.map(
      c => c.category
    );
...
function calculateSegmentStatistics(members, period) {
  ...
  const totalSpending = members.reduce(
    (sum, m) => sum + m.spending[spendingField],
    0
  );

  const totalLifetime = members.reduce(
    (sum, m) => sum + m.spending.lifetime,
    0
  );

  const totalPurchases = members.reduce(
    (sum, m) => sum + m.metadata.totalPurchases,
    0
  );
  ...
  return {
    ...
    medianSpending: calculateMedian(
      members.map(m => m.spending[spendingField])
    ),
  };
}
...
function calculateActivityStatistics(members, timeframe) {
  ...
  const totalSessions = members.reduce(
    (sum, m) => sum + (m.activity.sessionsThisMonth || 0),
    0
  );
...
async function applyComplexFilters(members, conditions) {
  ...
  if (conditions.purchaseConsistency) {
    filtered = filtered.filter(
      m => m.metadata.purchaseConsistency >= conditions.purchaseConsistency
    );
  }
  ...
  if (conditions.accountAge) {
    ...
    filtered = filtered.filter(m => new Date(m.metadata.joinDate) <= minDate);
  }

  // Filter by monthly active
  if (conditions.monthlyActive) {
    filtered = filtered.filter(
      m => m.activity.level === 'active' || m.activity.level === 'very-active'
    );
  }
  ...
  if (conditions.conversionRate) {
    filtered = filtered.filter(
      m => m.engagement.conversionRate >= conditions.conversionRate
    );
  }
...
function generateSpendingSummary(segments) {
  const totalMembers = Object.values(segments).reduce(
    (sum, segment) => sum + segment.count,
    0
  );

  const totalValue = Object.values(segments).reduce(
    (sum, segment) => sum + segment.statistics.totalSpending,
    0
  );
  ...
  return {
    ...
    totalValue,
    largestSegment: Object.entries(segments).sort(
      (a, b) => b[1].count - a[1].count
    )[0]?.[0],
    highestValueSegment: Object.entries(segments).sort(
      (a, b) => b[1].statistics.totalSpending - a[1].statistics.totalSpending
    )[0]?.[0],
  };
}
...
function analyzeSegmentBehavior(members) {
  return {
    avgResponseRate:
      members.reduce((sum, m) => sum + m.engagement.messageResponseRate, 0) /
      members.length,
    avgConversionRate:
      members.reduce((sum, m) => sum + m.engagement.conversionRate, 0) /
      members.length,
  };
}
...
function calculatePurchasingPatterns(members) {
  return {
    avgPurchaseFrequency:
      members.reduce((sum, m) => sum + m.metadata.totalPurchases, 0) /
      members.length,
    avgPurchaseValue:
      members.reduce((sum, m) => sum + m.spending.averagePurchase, 0) /
      members.length,
  };
}
...
function calculateSegmentValue(members) {
  const totalSpending = members.reduce(
    (sum, m) => sum + m.spending.last30Days,
    0
  );
...
function assessSegmentOpportunity(members, segmentName) {
  const growthPotential =
    members.filter(m => m.spending.velocity.trend === 'increasing').length /
    members.length;
...
function analyzeCrossSegments(segments) {
  ...
  return {
    multiSegmentMembers,
    avgSegmentsPerMember:
      Object.values(memberSegmentMap).reduce(
...
function generateOverallRecommendations(segments) {
  ...
  const highValueSegments = Object.entries(segments)
    .filter(
      ([name, data]) => data.priority === 'critical' || data.priority === 'high'
    )
  ...
  const growthSegments = Object.entries(segments)
    ...
    .sort(
      (a, b) =>
        b[1].opportunity.opportunityScore - a[1].opportunity.opportunityScore
    );
