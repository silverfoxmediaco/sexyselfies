...
const memberAnalyticsSchema = new mongoose.Schema({
  ...
  segments: {
    primary: {
      type: String,
      enum: ['whale', 'vip', 'regular', 'potential', 'at-risk', 'dormant', 'new'],
      default: 'new',
      index: true
    },
...
memberAnalyticsSchema.index({ 'segments.primary': 1, 'spending.last30Days': -1 });
memberAnalyticsSchema.index({ 'scoring.valueScore': -1, 'privacy.discoverable': 1 });
...
memberAnalyticsSchema.methods.calculateValueScore = function() {
  ...
  const activityScore = Math.min(100, (this.activity.monthlyActiveDays / 30) * 100);
...
memberAnalyticsSchema.methods.updateSpending = async function(amount, period = 'all') {
  const now = Date.now();
  ...
  this.spending.averagePurchase = this.spending.lifetime / this.metadata.totalPurchases;
...
memberAnalyticsSchema.statics.getBySegment = function(segment, creatorId = null) {
  const query = {
    'privacy.discoverable': true,
...
// Create model
const MemberAnalytics = mongoose.model('MemberAnalytics', memberAnalyticsSchema);
