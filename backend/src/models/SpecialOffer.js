...
const specialOfferSchema = new mongoose.Schema({
  ...
  recipients: {
    ...
    segments: [{
      type: String,
      enum: ['whale', 'vip', 'regular', 'potential', 'at-risk', 'returning', 'new']
    }],
  ...
  offer: {
    ...
    content: {
      ...
      items: [{
        ...
        contentType: {
          type: String,
          enum: ['photo', 'video', 'photoset', 'videoset', 'live', 'message']
        },
...
specialOfferSchema.index({ creator: 1, 'status.current': 1, 'validity.endDate': -1 });
specialOfferSchema.index({ 'recipients.members.member': 1, 'status.current': 1 });
...
specialOfferSchema.virtual('effectiveDiscount').get(function() {
  ...
  } else if (this.offer.type === 'fixed_discount' && this.offer.content.bundle) {
    return (this.offer.discount.fixedAmount / this.offer.content.bundle.totalValue) * 100;
  }
  return 0;
});
...
specialOfferSchema.methods.recordRedemption = async function(redemptionData) {
  const { memberId, transactionId, originalAmount, discountAmount, finalAmount } = redemptionData;
  ...
  if (this.performance.roi.cost > 0) {
    this.performance.roi.percentage = 
      ((this.performance.roi.revenue - this.performance.roi.cost) / this.performance.roi.cost) * 100;
...
specialOfferSchema.methods.calculatePerformanceScore = function() {
  ...
  const aovScore = Math.min(10, (this.performance.revenue.averageOrderValue / 100) * 10);
  score += aovScore;
...
specialOfferSchema.methods.canBeRedeemedBy = async function(memberId) {
  ...
  if (this.validity.usageLimit.total && 
...
specialOfferSchema.statics.getTopPerformers = async function(creatorId, limit = 10) {
  return this.find({
    creator: creatorId,
...
specialOfferSchema.statics.getSuggestedOffers = async function(creatorId, memberSegment) {
...
specialOfferSchema.pre('save', function(next) {
  ...
  } else if (this.validity.startDate <= now && 
...
specialOfferSchema.post('save', async function(doc) {
  if (doc.status.current === 'active' && doc.recipients.members.length > 0) {
    ...
    const pendingRecipients = doc.recipients.members.filter(r => r.status === 'pending');
    
    // This would integrate with notification service
    console.log(`Scheduling notifications for ${pendingRecipients.length} recipients`);
  }
});
