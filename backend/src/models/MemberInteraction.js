...
const memberInteractionSchema = new mongoose.Schema({
  ...
  message: {
    ...
    callToAction: {
      type: String,
      enum: ['none', 'view_profile', 'check_content', 'special_offer', 'subscribe'],
      default: 'none'
    }
  },
  ...
  specialOffer: {
    ...
    offerType: {
      type: String,
      enum: ['discount', 'bundle', 'exclusive', 'free_preview', 'time_limited']
    },
  ...
  compliance: {
    ...
    flags: [{
      type: String,
      enum: ['too_frequent', 'ignored_opt_out', 'inappropriate_content', 'reported_spam']
    }]
  },
...
memberInteractionSchema.index({ creator: 1, 'status.current': 1, createdAt: -1 });
memberInteractionSchema.index({ creator: 1, 'conversion.resulted_in_purchase': 1 });
memberInteractionSchema.index({ 'specialOffer.expiresAt': 1, 'specialOffer.wasRedeemed': 1 });
...
// Record response
memberInteractionSchema.methods.recordResponse = function(responseType, message = null) {
  const now = new Date();
...
memberInteractionSchema.methods.recordConversion = function(purchaseData) {
  ...
  if (this.effectiveness.roi.investment) {
    ...
    this.effectiveness.roi.percentage = 
      ((purchaseData.amount - this.effectiveness.roi.investment) / this.effectiveness.roi.investment) * 100;
  }
...
memberInteractionSchema.methods.calculateEffectiveness = function() {
  ...
  if (this.response.hasResponded) {
    score += 20;
    if (this.response.responseType === 'message' || this.response.responseType === 'purchase') {
      score += 10;
    }
  }
...
memberInteractionSchema.statics.getCreatorInteractions = function(creatorId, options = {}) {
...
memberInteractionSchema.statics.getMemberHistory = function(memberId, creatorId = null) {
...
memberInteractionSchema.statics.getConversionRate = async function(creatorId, days = 30) {
...
memberInteractionSchema.statics.getBestPerformingTypes = async function(creatorId) {
...
memberInteractionSchema.statics.checkDailyLimit = async function(creatorId, limit = 100) {
  ...
  try {
    ...
    if (analytics) {
      ...
      if (doc.conversion.resulted_in_purchase) {
        const creatorIndex = analytics.engagement.favoriteCreators
          .findIndex(fc => fc.creator.toString() === doc.creator.toString());
        
        if (creatorIndex >= 0) {
          analytics.engagement.favoriteCreators[creatorIndex].purchaseCount += 1;
          analytics.engagement.favoriteCreators[creatorIndex].totalSpent += doc.conversion.purchaseAmount;
        } else {
...
// Create model
const MemberInteraction = mongoose.model('MemberInteraction', memberInteractionSchema);
