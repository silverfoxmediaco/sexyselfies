...
memberProfileSchema.methods.assignBadges = function () {
  ...
  if (this.activity.messagesExchanged >= 100)
  ...
  if (this.spending.purchaseFrequency === 'daily')
...
memberProfileSchema.methods.calculateChurnRisk = function () {
  const daysSinceLastActive =
    (Date.now() - new Date(this.member.lastActive).getTime()) /
    (1000 * 60 * 60 * 24);
  const daysSinceLastPurchase = this.spending.lastPurchaseDate
    ? (Date.now() - new Date(this.spending.lastPurchaseDate).getTime()) /
      (1000 * 60 * 60 * 24)
    : 999;
