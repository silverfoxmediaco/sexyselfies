...
creatorAnalyticsSchema.methods.calculateTrendingScore = function () {
  ...
  this.realTime.trending.trendingScore = Object.values(factors).reduce(
    (a, b) => a + b,
    0
  );
...
creatorAnalyticsSchema.methods.detectAnomalies = function () {
  ...
  const avgDailyRevenue =
    this.historical.daily.slice(-7).reduce((sum, day) => sum + day.revenue, 0) /
    7;
  ...
  const platformAvg = await this.aggregate([
    {
      $group: {
        ...
        avgConversion: {
          $avg: '$funnels.browse.conversionRates.overallConversion',
        }, // CHANGED FROM: discovery
      },
    },
  ]);
