...
creatorSalesActivitySchema.index({
  creator: 1,
  'monthlyStats.year': -1,
...
// Record interaction
creatorSalesActivitySchema.methods.recordInteraction = async function (
  type,
  memberId,
  memberSegment
) {
  ...
  this.dailyActivity.interactions[type] =
    (this.dailyActivity.interactions[type] || 0) + 1;
...
// Record conversion
creatorSalesActivitySchema.methods.recordConversion = async function (
  revenue,
  memberId
) {
  ...
  this.dailyActivity.conversions.conversionRate =
    (this.dailyActivity.conversions.sales /
      this.dailyActivity.interactions.totalInteractions) *
    100;
  ...
  this.goals.daily.conversions.percentage =
    (this.goals.daily.conversions.achieved /
      this.goals.daily.conversions.target) *
    100;
...
creatorSalesActivitySchema.methods.calculatePerformanceScore = function () {
  ...
  const conversionScore = Math.min(
    30,
    this.dailyActivity.conversions.conversionRate * 3
  );
  ...
  const responseScore = Math.min(
    20,
    this.dailyActivity.responses.responseRate * 0.2
  );
  score += responseScore;
  ...
  const activityScore = Math.min(
    10,
    (this.dailyActivity.interactions.totalInteractions / 50) * 10
  );
  score += activityScore;
...
// Add achievement
creatorSalesActivitySchema.methods.unlockAchievement = function (
  type,
  name,
  description
) {
...
creatorSalesActivitySchema.methods.checkAchievements = function () {
  ...
  if (
    ...
    achievements.push(
      this.unlockAchievement('milestone', 'First Sale', 'Made your first sale!')
    );
  }

  // Daily goal
  if (
    this.goals.daily.revenue.percentage >= 100 &&
  ...
  // Whale hunter
  if (
    this.dailyActivity.segmentsEngaged.whales >= 5 &&
  ...
  // Conversion master
  if (
    this.dailyActivity.conversions.conversionRate >= 20 &&
  ...
  // Streak achievements
  if (
    this.rankings.streaks.currentDaily >= 7 &&
    !this.rankings.achievements.find(a => a.name === 'Week Warrior')
  ) {
    achievements.push(
      this.unlockAchievement('streak', 'Week Warrior', '7 day activity streak!')
    );
  }
...
// Get top performers
creatorSalesActivitySchema.statics.getTopPerformers = async function (
  limit = 10,
  timeframe = 'daily'
) {
  const sortField =
...
// Get creator's rank
creatorSalesActivitySchema.statics.getCreatorRank = async function (
  creatorId,
  metric = 'revenue'
) {
  const sortField =
  ...
  const rank =
    allCreators.findIndex(c => c.creator.toString() === creatorId.toString()) +
    1;
...
creatorSalesActivitySchema.statics.generateRecommendations = async function (
  creatorId
) {
  ...
  if (activity.dailyActivity.conversions.conversionRate < 5) {
    recommendations.push({
      ...
      description:
        'Your conversion rate is below 5%. Try more personalized messages and better timing.',
  ...
  if (activity.dailyActivity.segmentsEngaged.whales === 0) {
    recommendations.push({
      ...
      description:
        "You haven't engaged any whale members today. They convert 3x better!",
  ...
  if (activity.dailyActivity.responses.responseRate < 10) {
    recommendations.push({
      ...
      description:
        'Your response rate is low. Try asking questions and being more personal.',
  ...
  if (activity.dailyActivity.interactions.specialOffers === 0) {
    recommendations.push({
      ...
      description:
        'Special offers convert 5x better than regular messages. Try one today!',
...
creatorSalesActivitySchema.pre('save', function (next) {
  ...
  if (
    this.metadata.lastActivityAt &&
    this.metadata.lastActivityAt.toDateString() === today.toDateString()
  ) {
    this.rankings.streaks.currentDaily += 1;
    if (
      this.rankings.streaks.currentDaily > this.rankings.streaks.longestDaily
    ) {
...
// Create model
const CreatorSalesActivity = mongoose.model(
  'CreatorSalesActivity',
  creatorSalesActivitySchema
);
