...
const creatorMessageSchema = new mongoose.Schema({
  ...
  content: {
    type: {
      type: String,
      enum: ['text', 'photo', 'video', 'audio', 'gif', 'voice', 'tip', 'system'],
      required: true
    },
...
creatorMessageSchema.methods.canView = function(userId) {
  ...
  if (this.purchase.status === 'unlocked') {
    if (this.pricing.expiringContent.enabled) {
      return this.purchase.purchasedAt && 
             Date.now() < new Date(this.purchase.purchasedAt).getTime() + 
...
// Process purchase
creatorMessageSchema.methods.processPurchase = async function(buyerId, transactionId) {
  if (this.purchase.status === 'unlocked') {
  ...
  if (this.pricing.expiringContent.enabled) {
    ...
    this.pricing.expiringContent.expiresAt = new Date(Date.now() + expirationMs);
  }
...
creatorMessageSchema.statics.getConversation = function(creatorId, memberId, options = {}) {
  const query = {
    $or: [
...
creatorMessageSchema.statics.findHighValueMessages = function(creatorId, minValue = 5) {
  return this.find({
    creator: creatorId,
...
creatorMessageSchema.statics.getAutomationPerformance = async function(creatorId, campaignId) {
  const stats = await this.aggregate([
    {
      $match: {
        creator: creatorId,
