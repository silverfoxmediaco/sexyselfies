...
creatorConnectionSchema.methods.calculateRelationshipStrength = function () {
  const factors = {
    ...
    recency:
      this.engagement.lastActiveAt > Date.now() - 7 * 24 * 60 * 60 * 1000
        ? 0.2
        : 0,
...
creatorConnectionSchema.methods.updateRelationshipHealth = function () {
  const daysSinceInteraction =
    (Date.now() - this.engagement.lastActiveAt) / (1000 * 60 * 60 * 24);
...
creatorConnectionSchema.statics.findCreatorConnections = function (
  creatorId,
  options = {}
) {
...
creatorConnectionSchema.statics.findTopSpenders = function (
  creatorId,
  limit = 10
) {
  return this.find({
