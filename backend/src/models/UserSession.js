...
UserSessionSchema.methods.addActivity = async function (
  activityType,
  metadata = {}
) {
...
UserSessionSchema.methods.updateMetricsForActivity = function (
  activityType,
  metadata
) {
  switch (activityType) {
...
UserSessionSchema.methods.flagSuspiciousActivity = function (
  ...
  this.security.riskScore = Math.min(
    100,
    this.security.riskScore + scoreIncrease
  );
...
UserSessionSchema.statics.endAllUserSessions = function (
  userId,
  reason = 'security'
) {
...
UserSessionSchema.statics.getSessionAnalytics = async function (
  timeframe = 30
) {
