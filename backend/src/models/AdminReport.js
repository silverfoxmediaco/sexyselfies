...
const adminReportSchema = new mongoose.Schema({
  ...
  status: {
    type: String,
    enum: ['pending', 'under_review', 'resolved', 'dismissed', 'escalated', 'appealed'],
    default: 'pending'
  },
  assignedTo: {
    type: mongoose.Schema.Types.ObjectId,
  ...
  metadata: {
    source: {
      type: String,
      enum: ['user_report', 'ai_detection', 'manual_review', 'automated_scan'],
      default: 'user_report'
    },
    aiConfidence: Number,
...
adminReportSchema.pre('save', function(next) {
  if (this.isNew) {
    ...
    if (this.decision && this.decision.action && 
...
adminReportSchema.methods.resolve = async function(adminId, decision) {
  ...
  const seriousActions = ['user_suspended_7d', 'user_suspended_30d', 'user_banned', 'content_removed'];
