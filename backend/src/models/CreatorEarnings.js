...
// METHODS
creatorEarningsSchema.methods.calculateGrowthRate = function(timeframe = 'monthly') {
...
creatorEarningsSchema.methods.predictNextPeriod = function(timeframe = 'daily') {
  ...
  const average = recentPeriods.reduce((sum, p) => sum + p.amount, 0) / recentPeriods.length;
  
  // Calculate trend
  const firstHalf = recentPeriods.slice(0, 3).reduce((sum, p) => sum + p.amount, 0) / 3;
  const secondHalf = recentPeriods.slice(-3).reduce((sum, p) => sum + p.amount, 0) / 3;
...
creatorEarningsSchema.methods.identifyTopOpportunities = function() {
  ...
  const bestContent = this.earningsBreakdown.byContentType.sort((a, b) => b.averagePrice - a.averagePrice)[0];
  if (bestContent) {
    opportunities.push({
      type: 'content',
...
creatorEarningsSchema.virtual('needsAttention').get(function() {
  const warnings = this.insights.warnings.filter(w => w.severity === 'critical');
