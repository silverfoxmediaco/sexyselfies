...
userViolationSchema.methods.addStrike = async function(reason, adminId, duration = 90) {
...
userViolationSchema.methods.addWarning = async function(message, severity, adminId) {
  this.warnings.push({
    message: message,
    severity: severity,
    issuedBy: adminId,
...
userViolationSchema.methods.freezePayouts = async function(days, adminId) {
  this.restrictions.payoutsFrozen = true;
  this.restrictions.frozenUntil = new Date(Date.now() + days * 24 * 60 * 60 * 1000);
...
userViolationSchema.methods.calculateRiskScore = async function() {
  ...
  const recentViolations = this.violations.filter(v => 
    v.issuedAt > new Date(Date.now() - 30 * 24 * 60 * 60 * 1000)
  );
