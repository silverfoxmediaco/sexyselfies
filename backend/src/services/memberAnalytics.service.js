 ...
 */
async function updateMemberPurchaseAnalytics(memberId, amount, transactionType, creatorId = null) {
  try {
    console.log(`📊 Updating analytics for member ${memberId}: $${amount} ${transactionType}`);
    ...
    analytics.spending.averagePurchase = analytics.spending.lifetime / analytics.spending.totalTransactions;
    ...
    if (creatorId) {
      ...
      analytics.metadata.favoriteCreators.sort((a, b) => b.totalSpent - a.totalSpent);
      analytics.metadata.favoriteCreators = analytics.metadata.favoriteCreators.slice(0, 10);
    }
    
    // Calculate new tier based on spending
    analytics.spending.tier = calculateSpendingTier(analytics.spending.last30Days);
    ...
    console.log(`✅ Updated analytics for ${memberId}: Tier ${analytics.spending.tier}, Score ${analytics.scoring.valueScore}`);
...
function calculateActivityLevel(analytics) {
  ...
  const daysSinceJoin = (Date.now() - new Date(analytics.metadata.joinDate)) / (1000 * 60 * 60 * 24);
...
function calculateMemberScoring(analytics) {
  const { last30Days, lifetime, averagePurchase, totalTransactions } = analytics.spending;
  const { contentUnlocks, tipsGiven } = analytics.engagement;
  ...
  const daysSinceJoin = (Date.now() - new Date(analytics.metadata.joinDate)) / (1000 * 60 * 60 * 24);
  const daysSinceLastPurchase = (Date.now() - new Date(analytics.spending.lastPurchaseDate)) / (1000 * 60 * 60 * 24);
