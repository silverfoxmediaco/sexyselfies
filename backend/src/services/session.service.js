...
class SessionService {
  ...
  static async validateSession(sessionId, userId, req) {
    try {
      ...
      if (session.location.ipAddress !== currentIp) {
        console.log(
          `🚨 IP change detected for session ${sessionId}: ${session.location.ipAddress} → ${currentIp}`
        );
        ...
        await session.security.ipChanges.push({
          ...
          toIp: currentIp,
          timestamp: new Date(),
          flagged: this.shouldFlagIpChange(
        ...
        if (
          newLocation.country &&
          newLocation.country !== session.location.country
        ) {
      ...
      if (inactiveMinutes > 30) {
        // 30 minutes inactivity threshold
        console.log(
          `⏰ Session ${sessionId} was inactive for ${inactiveMinutes} minutes`
        );
      }
  ...
  static async trackActivity(sessionId, activityType, metadata = {}) {
    try {
      ...
      if (!session) {
        console.log(
          `❌ Cannot track activity - session not found: ${sessionId}`
        );
        return false;
      }
      ...
      console.log(
        `📊 Activity tracked: ${activityType} for session ${sessionId}`
      );
      return true;
    } catch (error) {
  ...
  static async endAllUserSessions(userId, reason = 'security') {
    try {
      ...
      console.log(
        `🔒 Ended ${result.modifiedCount} sessions for user ${userId}`
      );
  ...
  static async cleanupExpiredSessions() {
    try {
      ...
      console.log(
        `✅ Session cleanup complete: ${result.modifiedCount} sessions marked as expired`
      );
      return result.modifiedCount;
    } catch (error) {
