  ...
  io.on('connection', (socket) => {
    ...
    socket.on('join_chat', async (data) => {
      try {
        ...
        if (socket.userRole === 'member' && connection.member.toString() === socket.profile._id.toString()) {
          hasAccess = true;
        } else if (socket.userRole === 'creator' && connection.creator.toString() === socket.profile._id.toString()) {
          hasAccess = true;
        }
    ...
    socket.on('send_message', async (data) => {
      try {
        ...
        const messageData = {
          ...
          senderName: socket.userRole === 'member' ? 'Member' : socket.profile.displayName,
        ...
        console.log(`💬 Message sent in chat ${connectionId} by ${socket.user.email}`);
    ...
    socket.on('mark_messages_read', async (data) => {
      try {
        ...
        console.log(`💬 Messages marked as read in chat ${connectionId} by ${socket.user.email}`);
