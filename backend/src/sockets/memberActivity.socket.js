  ...
  memberNamespace.on('connection', (socket) => {
    ...
    socket.on('member:online', async (data) => {
      try {
        ...
        if (analytics && analytics.spending.tier === 'whale' || analytics.spending.tier === 'vip') {
    ...
    socket.on('member:responded', async (data) => {
      try {
        ...
        if (interaction) {
          // Notify creator immediately
          memberNamespace.to(`creator:${interaction.creator}`).emit('member:response', {
            memberId,
            interactionId,
            responseType,
    ...
    socket.on('activity:content-interaction', async (data) => {
      try {
        ...
        if (content) {
          memberNamespace.to(`creator:${content.creator}`).emit('content:interaction', {
            memberId,
            contentId,
            action,
            timestamp: new Date()
...
async function subscribeToHighValueMembers(socket, creatorId) {
  try {
    ...
    const creator = await Creator.findById(creatorId)
      .select('preferences.notifyHighValueMembers');
...
async function notifyRelevantCreators(memberId, eventType, data) {
  try {
    ...
    interactions.forEach(creatorId => {
      io.of('/member-activity').to(`creator:${creatorId}`).emit(`member:${eventType}`, {
        memberId,
        ...data
      });
    });
