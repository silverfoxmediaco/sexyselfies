...
async function generateUniqueUsername(displayName) {
  ...
  do {
    ...
    if (attempts > 10) {
      username = generateUsername(displayName) + Date.now().toString().slice(-4);
    }
  ...
  try {
    ...
    for (const creator of creators) {
      try {
        ...
        console.error(`❌ Failed to update ${creator.displayName}:`, error.message);
        failed++;
      }
    }
    ...
    console.log('\\n🔍 Verification:');
    console.log(`📈 Creators with usernames: ${creatorsWithUsernames}/${totalCreators}`);
