...
exports.getSwipeStack = async (req, res) => {
  try {
    ...
    if (ethnicity !== 'all') {
      query.ethnicity = Array.isArray(ethnicity) ? { $in: ethnicity } : ethnicity;
    }
    
    // Get location-based creators if distance specified
    let creators;
    if (distance !== 'anywhere' && member.location && member.location.coordinates) {
    ...
    // Enrich creator data
    const enrichedStack = await Promise.all(stack.map(async (creator) => {
      ...
      return {
        ...
        location: {
          ...
          distance: creator.distance ? Math.round(creator.distance / 1609.34) : null
        },
...
exports.swipe = async (req, res) => {
  try {
    ...
    if (direction === 'right' || direction === 'super') {
      ...
      const creatorProfile = await CreatorProfile.findOne({ creator: creatorId });
    ...
    res.json({
      ...
      message: isConnected ? 
        ...
        (direction === 'right' ? 'Like sent!' : 'Passed')
    });
  } catch (error) {
  ...
  try {
    ...
    res.json({
      ...
      stats: {
        ...
        todayLikes: enrichedLikes.filter(l => 
          new Date(l.likedAt).toDateString() === new Date().toDateString()
        ).length
      }
    });
  } catch (error) {
...
exports.getConnections = async (req, res) => {
  try {
    ...
    if (filter === 'active') {
      ...
      query.connectedAt = { $gte: new Date(Date.now() - 7 * 24 * 60 * 60 * 1000) };
    ...
    const enrichedConnections = await Promise.all(connections.map(async (connection) => {
      const otherUser = userType === 'creator' ? connection.member : connection.creator;
      ...
      return {
        ...
        engagement: {
          totalMessages: connection.engagement.totalMessages.fromCreator + 
        ...
        notifications: {
          muted: !connection.notifications[userType + 'Preferences'].newMessage
        }
      };
    }));
    ...
    res.json({
      ...
      summary: {
        total,
        active: enrichedConnections.filter(c => 
          c.relationship.status === 'active' || c.relationship.status === 'thriving'
        ).length,
        atRisk: enrichedConnections.filter(c => c.relationship.churnRisk >= 70).length,
        new: enrichedConnections.filter(c => 
...
exports.disconnect = async (req, res) => {
  try {
    ...
    if (connection.creator.toString() !== userId && connection.member.toString() !== userId) {
  ...
  try {
    ...
    const analytics = {
      overview: {
        ...
        rightSwipes: allConnections.filter(c => 
          c.swipeData.memberSwiped.direction === 'right' || 
          c.swipeData.memberSwiped.direction === 'super'
        ).length,
        superLikes: allConnections.filter(c => c.swipeData.memberSwiped.superLike).length,
        connections: allConnections.filter(c => c.status === 'connected').length,
        activeConnections: activeConnections.length
      },
      
      conversionRates: {
        swipeToConnection: calculateConversionRate(
          allConnections.filter(c => c.swipeData.memberSwiped.direction === 'right').length,
          allConnections.filter(c => c.status === 'connected').length
        ),
        connectionToMessage: calculateConversionRate(
          allConnections.filter(c => c.status === 'connected').length,
          allConnections.filter(c => c.engagement.totalMessages.fromMember > 0).length
        ),
      ...
      revenue: {
        fromConnections: activeConnections.reduce((sum, c) => sum + c.monetization.totalRevenue, 0),
        averagePerConnection: activeConnections.length > 0 ?
          activeConnections.reduce((sum, c) => sum + c.monetization.totalRevenue, 0) / activeConnections.length : 0,
      ...
      engagement: {
        avgMessagesPerConnection: calculateAverage(
          activeConnections.map(c => 
            c.engagement.totalMessages.fromCreator + c.engagement.totalMessages.fromMember
          )
        ),
...
exports.updateConnectionPreferences = async (req, res) => {
  try {
    ...
    const userType = connection.creator.toString() === userId ? 'creator' : 'member';
...
async function applySmartAlgorithm(creators, member, filters) {
  ...
  while (scoredCreators.length > 0) {
    const index = Math.random() < 0.7 ? 0 : Math.floor(Math.random() * Math.min(5, scoredCreators.length));
...
function calculateCompatibility(member, creator) {
  ...
  if (isOrientationCompatible(member.orientation, creator.orientation, member.gender, creator.gender)) {
    score += 30;
  }
  
  // Interest overlap
  const commonInterests = member.interests?.filter(i => 
    creator.interests?.includes(i)
  ).length || 0;
...
function isOrientationCompatible(memberOri, creatorOri, memberGender, creatorGender) {
...
async function trackBrowseSession(memberId, stack, filters) {
  ...
  console.log(`Browse session for member ${memberId}: ${stack.length} creators shown`);
}
...
async function notifyCreatorOfSuperLike(creatorId, memberId) {
  const member = await Member.findById(memberId).select('username profileImage');
...
async function updateSwipeAnalytics(creatorId, memberId, direction, isConnected) {
...
async function handleNewConnection(connection, instantConnection) {
  ...
  const creator = await Creator.findById(connection.creator).select('username profileImage');
  const member = await Member.findById(connection.member).select('username profileImage');
  
  // Notify member
  await sendNotification(connection.member, {
    type: 'new_connection',
    title: instantConnection ? 'Instant Connection! 🎉' : "It's a Connection! 💕",
...
function calculateResponseRate(connections) {
  const responded = connections.filter(c => c.engagement.totalMessages.fromCreator > 0).length;
...
function generateConnectionInsights(analytics, activeConnections) {
  ...
  if (analytics.overview.superLikes > 5) {
    insights.push('You received ' + analytics.overview.superLikes + ' super likes this period!');
  }
  
  if (analytics.conversionRates.connectionToPurchase < 20) {
    insights.push('Tip: Engage with connections quickly to increase conversions');
  }
  
  if (analytics.revenue.averagePerConnection < 10) {
    insights.push('Consider offering exclusive content to increase connection value');
  }
