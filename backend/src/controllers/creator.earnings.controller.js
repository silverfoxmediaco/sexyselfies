...
exports.getEarningsDashboard = async (req, res) => {
  try {
    ...
    const realtimeEarnings = await calculateRealtimeEarnings(
      creatorId,
      periodDates
    );
    ...
    const dashboard = {
      ...
      realtime: {
        ...
        notifications: generateEarningsNotifications(
          realtimeEarnings,
          earnings
        ),
      },
...
exports.getTransactions = async (req, res) => {
  try {
    ...
    if (customerId) {
      transactions = transactions.filter(
        t => t.customer.toString() === customerId
      );
    }
      ...
      paginatedTransactions.map(async transaction => {
        const enriched = {
          ...
          contentInfo: transaction.reference
            ? await getContentInfo(transaction.reference, transaction.type)
            : null,
        };
  ...
  try {
    ...
    if (earnings.payouts.instantPayoutEnabled && method === 'instant') {
      try {
        ...
        const transfer = await processInstantPayoutCCBill(
          creatorId,
          amount,
          accountDetails
        );
...
exports.getPayoutHistory = async (req, res) => {
  try {
    ...
    const payouts = earnings.payouts.history.sort(
...
async function calculateRealtimeEarnings(creatorId, periodDates) {
  ...
  const total =
    (contentEarnings[0]?.total || 0) + (messageEarnings[0]?.total || 0);
...
function calculateMilestones(earnings) {
  ...
  const progress = nextMilestone
    ? ((total / nextMilestone.amount) * 100).toFixed(2)
    : 100;
...
function generateActionItems(earnings) {
  return [
    {
      priority: 'high',
      action: 'Re-engage top spenders showing churn signals',
    },
