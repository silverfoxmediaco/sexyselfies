...
router.get(
  ...
  async (req, res) => {
    try {
      const { limit = 20, category, orientation, gender } = req.query;
      ...
      const creators = trendingAnalytics.map(analytics => ({
        ...
        stats: {
          contentCount:
            analytics.contentPerformance.byType.photos.total +
...
router.get('/creator', cacheMiddleware(300), async (req, res) => {
  try {
    const {
      page = 1,
      limit = 20,
    ...
    const creators = await Creator.find(query)
      .select(
        'username profileImage bio age location category orientation gender isVerified stats'
      )
      .sort(sort)
      .skip(skip)
      .limit(parseInt(limit));
    ...
    const enrichedCreators = await Promise.all(
      creators.map(async creator => {
        const profile = await CreatorProfile.findOne({
          ...
          'branding.displayName branding.tagline preferences.pricing analytics.performance.rating'
        );
...
router.get('/creator/:identifier', cacheMiddleware(300), async (req, res) => {
  try {
    ...
    const creator = await Creator.findOne(query).select(
      '-password -email -phoneNumber -earnings -paymentInfo'
    );
    ...
    const profile = await CreatorProfile.findOne({
      ...
      'branding personalization preferences.contentTypes analytics.performance'
    );
    ...
    const sampleContent = await Content.find({
      ...
      .select(
        'title contentType media.thumbnailUrl pricing analytics.totalViews createdAt'
      )
      .sort('-createdAt')
      .limit(9);
...
router.get('/stats', cacheMiddleware(3600), async (req, res) => {
  try {
    const stats = {
      creators: {
        total: await Creator.countDocuments({ isActive: true }),
        verified: await Creator.countDocuments({
          isActive: true,
          isVerified: true,
        }),
      ...
      members: {
        total: await require('../models/Member').countDocuments({
          isActive: true,
        }),
      },
    };
        ...
        sections: [
          {
            ...
            content:
              'By accessing and using SexySelfies, you agree to be bound by these Terms of Service...',
          },
          {
          ...
          },
          {
            ...
            content:
              'Content must comply with our community guidelines. No explicit nudity...',
          },
          {
            title: 'Payment Terms',
            content:
              'All payments are processed securely. Platform fee is 20%...',
          },
          {
            title: 'Creator Obligations',
            content:
              'Creators must verify their identity and maintain accurate information...',
          },
        ],
      },
    });
...
// Privacy Policy
router.get('/privacy', cacheMiddleware(86400), (req, res) => {
  res.json({
    success: true,
    content: {
...
router.get('/guidelines', cacheMiddleware(86400), (req, res) => {
  res.json({
    success: true,
    content: {
      ...
      sections: [
        {
          ...
          content:
            'Content should be sexy but not pornographic. Lingerie and implied nudity allowed...',
        },
        {
          title: 'Respectful Behavior',
          content:
            'Treat all users with respect. Harassment will not be tolerated...',
        },
        {
          title: 'Prohibited Content',
          content:
            'No explicit nudity, violence, hate speech, or illegal content...',
        },
        {
          title: 'Reporting',
...
// FAQ
router.get('/faq', cacheMiddleware(86400), (req, res) => {
  res.json({
    success: true,
    faqs: [
      {
        ...
        answer:
          'SexySelfies is a swipe-to-match platform where you can discover creators and unlock their content...',
      },
      {
        question: 'How much does it cost?',
        answer:
          'There are no subscriptions! You only pay for the content you want, starting at $0.99...',
      },
      {
        question: 'How do I become a creator?',
        answer:
          'Sign up as a creator, verify your identity, and start posting content...',
      },
      {
        question: 'How do creators get paid?',
        answer:
          'Creators receive 80% of all earnings, with payouts available weekly or instantly...',
      },
      {
        question: 'Is my information secure?',
        answer:
          'Yes, we use industry-standard encryption and never store payment details...',
      },
    ],
  });
});
...
router.get('/config', cacheMiddleware(300), (req, res) => {
  res.json({
    success: true,
    config: {
      appName: 'SexySelfies',
