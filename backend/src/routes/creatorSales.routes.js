    ...
    try {
      ...
      switch(type) {
        ...
        case 'activity':
          ...
          break;
        case 'preferences':
          segments = await segmentByPreferences({ creatorId, matchCreatorContent: true });
          break;
        case 'custom':
    ...
    try {
      ...
      res.json({
        ...
        data: {
          ...
          bestPerforming: {
            interactionType: Object.entries(salesActivity.performance.conversionRates)
  ...
  async (req, res) => {
    try {
      ...
      const position = allCreators.findIndex(c => 
        c.creator._id.toString() === creatorId
      ) + 1;
      ...
      res.json({
        ...
        data: {
          ...
          topPerformers,
          nearbyCreators: position ? allCreators.slice(
...
function getNextMilestone(gamification) {
  ...
  return milestones.find(m => m.points > points) || {
    points: 999999,
