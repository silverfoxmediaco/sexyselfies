        ...
        levels: [
          {
            value: 'public',
            name: 'Public',
            description: 'Your profile is fully discoverable with basic info visible',
            features: [
    ...
    try {
      ...
      const visibilityStatus = {
        ...
        visibleTo: analytics.privacy.discoverable ? 
          (analytics.privacy.level === 'public' ? 'all_creators' : 'verified_creators') : 
          'none',
        spendingVisible: analytics.privacy.showSpending,
        activityVisible: analytics.privacy.level !== 'strict' && analytics.privacy.level !== 'private',
...
router.get('/report',
  ...
  async (req, res) => {
    try {
      ...
      const report = {
        profileViews: interactions.filter(i => i.interactionType === 'profile_view'),
        messages: interactions.filter(i => i.interactionType === 'message'),
        pokes: interactions.filter(i => i.interactionType === 'poke'),
        specialOffers: interactions.filter(i => i.interactionType === 'special_offer')
      };
      ...
      res.json({
        ...
        data: {
          ...
          summary: {
            ...
            uniqueCreators: new Set(interactions.map(i => i.creator._id.toString())).size,
...
function getPrivacyRecommendations(privacy) {
  ...
  if (privacy.level === 'public') {
    recommendations.push({
      type: 'warning',
      message: 'Your profile is fully public. Consider increasing privacy for better protection.'
    });
  }
  ...
  if (privacy.showSpending) {
    recommendations.push({
      type: 'info',
      message: 'Your spending data is visible to creators. You can hide this in visibility settings.'
    });
  }
  ...
  if (privacy.allowBulkMessages) {
    recommendations.push({
      type: 'info',
      message: 'You are receiving bulk messages. You can opt out in interaction preferences.'
    });
  }
...
async function collectMemberData(memberId) {
  ...
  // Basic profile
  data.profile = await Member.findById(memberId)
...
function getMostActiveCreator(interactions) {
  ...
  const mostActiveId = Object.entries(creatorCounts)
    .sort((a, b) => b[1] - a[1])[0]?.[0];
  
  if (!mostActiveId) return null;
  
  const creator = interactions.find(i => 
    i.creator._id.toString() === mostActiveId
  )?.creator;
