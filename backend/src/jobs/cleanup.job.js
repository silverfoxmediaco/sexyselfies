        ...
        for (const offer of expiredOffers) {
          try {
            ...
            if (offer.performance) {
              offer.performance.finalRedemptionRate =
                offer.performance.redemptionRate;
        ...
        while (hasMore) {
          ...
          for (const interaction of oldInteractions) {
            try {
              ...
              await ArchivedInteraction.create({
                ...
                summary: {
                  ...
                  resulted_in_purchase:
                    interaction.conversion.resulted_in_purchase,
              ...
              console.error(
                `Error archiving interaction ${interaction._id}:`,
                error.message
              );
            }
          }
          ...
          console.log(
            `  Archived batch of ${oldInteractions.length} interactions...`
          );
        }
  ...
  return cron.schedule(
    ...
    async () => {
      ...
      try {
        ...
        console.log(
          `  Deleted ${transactionResult.deletedCount} old failed transactions`
        );
        optimizations++;
        ...
        console.log(
          `  Deleted ${emptySalesResult.deletedCount} empty sales activities`
        );
        optimizations++;
...
async function updateMemberLifecycleStages() {
  const stages = [
    ...
    {
      name: 'activated',
      condition: { 'metadata.totalPurchases': { $gte: 1, $lt: 3 } },
    },
    {
      name: 'engaged',
      condition: { 'metadata.totalPurchases': { $gte: 3, $lt: 10 } },
    },
...
async function cleanupOrphanedAnalytics() {
  ...
  const orphanedAnalytics = analytics.filter(
    a => !existingMemberIds.has(a.member.toString())
  );
  ...
  if (orphanedAnalytics.length > 0) {
    ...
    console.log(
      `  Deleted ${orphanedAnalytics.length} orphaned analytics records`
    );
  }
}
...
async function rebuildIndexesIfNeeded() {
  ...
  for (const Model of collections) {
    try {
      ...
      console.error(
        `  Error checking indexes for ${Model.collection.name}:`,
        error.message
      );
    }
  }
