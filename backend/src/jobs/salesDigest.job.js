      ...
      try {
        ...
        for (const creator of creators) {
          try {
            ...
            console.error(
              `Error generating report for creator ${creator._id}:`,
              error.message
            );
            errors++;
          }
  ...
  return cron.schedule(
    ...
    async () => {
      ...
      try {
        ...
        const salesActivities = await CreatorSalesActivity.find({}).populate(
          'creator',
        ...
        for (const activity of salesActivities) {
          try {
            ...
            console.error(
              `Error generating weekly summary for creator ${activity.creator._id}:`,
              error.message
            );
          }
        }
      ...
      try {
        ...
        for (const creator of verifiedCreators) {
          try {
            ...
            const newHighValueMembers = highValueMembers.filter(
              m => !recentAlerts.includes(m.member._id.toString())
            );
            ...
            await recordAlert(
              creator._id,
              newHighValueMembers.map(m => m.member._id)
            );

            alertsSent++;
          } catch (error) {
            console.error(
              `Error sending high-value alert to creator ${creator._id}:`,
              error.message
            );
          }
        }
      ...
      try {
        ...
        for (const creator of creators) {
          try {
            ...
            const creatorOpportunities = await identifyOpportunities(
              creator._id
            );
            ...
            const highPriority = creatorOpportunities.filter(
            ...
            console.error(
              `Error identifying opportunities for creator ${creator._id}:`,
              error.message
            );
          }
        }
...
async function generateDailyReport(creatorId, salesActivity) {
  ...
  return {
    ...
    topPerformingContent: await getTopPerformingContent(
      creatorId,
      yesterday,
      today
    ),
    comparison: {
      revenueTrend: calculateTrend(
        ...
        salesActivity.daily.previousRevenue || 0
      ),
      conversionTrend: calculateTrend(
        salesActivity.daily.conversions,
        salesActivity.daily.previousConversions || 0
      ),
    },
  };
}
...
function compareWeeklyPerformance(activity) {
  ...
  return {
    ...
    interactionChange: calculatePercentageChange(
    ...
    trends: {
      ...
      conversions:
        current.totalConversions > previous.totalConversions ? 'up' : 'down',
      interactions:
        current.totalInteractions > previous.totalInteractions ? 'up' : 'down',
    },
  };
}
...
async function identifyOpportunities(creatorId) {
  ...
  if (inactiveWhales.length > 0) {
    opportunities.push({
      ...
      potentialValue: inactiveWhales.reduce(
