        ...
        while (hasMore) {
          ...
          for (const analytics of memberAnalytics) {
            try {
              ...
              console.error(
                `Error updating spending for member ${analytics.member._id}:`,
                error.message
              );
              errors++;
            }
      ...
      try {
        ...
        for (const [level, threshold] of Object.entries(activityThresholds)) {
          const cutoffDate = new Date(
            now - threshold.days * 24 * 60 * 60 * 1000
          );
          ...
          if (result.modifiedCount > 0) {
            ...
            console.log(
              `  Updated ${result.modifiedCount} members to ${level}`
            );
          }
        }
        ...
        for (const analytics of memberAnalytics) {
          try {
            ...
            if (oldTier !== newTier) {
              ...
              analytics.metadata.tierHistory =
                analytics.metadata.tierHistory || [];
              ...
              if (
                (newTier === 'whale' || newTier === 'vip') &&
                oldTier !== newTier
              ) {
                await notifyCreatorsOfHighValueMember(
                  analytics.member,
                  newTier
                );
              }
            ...
            console.error(
              `Error recalculating segment for member ${analytics.member}:`,
              error.message
            );
          }
        }
      ...
      try {
        ...
        for (const analytics of memberAnalytics) {
          try {
            ...
            console.error(
              `Error assessing risk for member ${analytics.member._id}:`,
              error.message
            );
          }
        }
