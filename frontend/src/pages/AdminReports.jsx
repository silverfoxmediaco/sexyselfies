...
const AdminReports = () => {
  ...
  const handleAction = async action => {
    ...
    try {
      const response = await api.put(
        `/admin/moderation/reports/${selectedReport._id}/resolve`,
        {
          action: action,
  ...
  const getSeverityColor = severity => {
    switch (severity) {
      case 'critical':
        return 'admin-reports-severity-critical';
      case 'high':
        return 'admin-reports-severity-high';
      case 'medium':
        return 'admin-reports-severity-medium';
      case 'low':
        return 'admin-reports-severity-low';
      default:
        return '';
    }
  };
  ...
  const getStatusColor = status => {
    switch (status) {
      ...
      case 'resolved':
        return 'admin-reports-status-resolved';
      case 'dismissed':
        return 'admin-reports-status-dismissed';
      default:
        return '';
    }
  };
    ...
    <>
      ...
      <div className='admin-reports-container'>
        ...
        <div className='admin-reports-header'>
          <div>
            ...
            <p className='admin-reports-subtitle'>
              {reports.length} {filter !== 'all' ? filter : ''} reports
            </p>
          </div>
          ...
          <div className='admin-reports-header-controls'>
            ...
            <button
              className='admin-reports-refresh-btn'
          ...
          <div className='admin-reports-list'>
            {reports.length === 0 ? (
              <div className='admin-reports-empty-state'>
                ...
                >
                  <path
                    d='M12 9v2m0 4h.01M3 7l9-4 9 4v7c0 5-4 9-9 9s-9-4-9-9V7z'
                    strokeWidth='2'
                  />
                </svg>
                ...
                >
                  <div className='admin-reports-main'>
                    <div className='admin-reports-header-info'>
                      <span
                        className={`admin-reports-severity-badge ${getSeverityColor(report.severity)}`}
                      >
                        {report.severity}
                      </span>
                      <span
                        className={`admin-reports-status-badge ${getStatusColor(report.status)}`}
                      >
                        {report.status}
                      </span>
                    </div>
                  ...
                  {report.reportedUser && (
                    <div className='admin-reports-reported-user'>
                      ...
                      <span className='admin-reports-user-email'>
                ...
                {selectedReport.reportedUser && (
                  <div className='admin-reports-detail-section'>
                    ...
                    <div className='admin-reports-info-grid'>
                      ...
                      <div className='admin-reports-info-item'>
                        <label>User ID</label>
                        <p className='admin-reports-user-id'>
                      ...
                      {selectedReport.reportedUser.strikes !== undefined && (
                        <div className='admin-reports-info-item'>
                          ...
                          <p className='admin-reports-strikes'>
                            {selectedReport.reportedUser.strikes}/3
                          </p>
                        </div>
                      )}
                    </div>
                      ...
                      {selectedReport.reportedContent.type === 'image' ? (
                        ...
                        <div className='admin-reports-content-info'>
                          ...
                          <p>
                            ...
                            {selectedReport.reportedContent._id}
                          </p>
                    ...
                    <div className='admin-reports-detail-section'>
                      ...
                      <div className='admin-reports-action-history'>
                        {selectedReport.actionHistory.map((action, index) => (
                          <div
                            key={index}
                            className='admin-reports-history-item'
                          >
                            <span className='admin-reports-history-action'>
                              {action.action}
                            </span>
                            <span className='admin-reports-history-admin'>
        ...
        {showActionModal && (
          ...
          >
            <div
              className='admin-reports-modal-content'
              onClick={e => e.stopPropagation()}
            >
              <h2>
                Confirm Action: {currentAction.replace('_', ' ').toUpperCase()}
              </h2>
