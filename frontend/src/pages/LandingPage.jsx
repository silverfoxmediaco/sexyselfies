...
export default function LandingPage({ logoSrc, onJoinWaitlist }) {
  ...
  const launchDate = React.useMemo(
    () => new Date('2025-09-15T17:00:00-05:00'),
    []
  );
  ...
  React.useEffect(() => {
    const s = setInterval(
      () => setSpotsLeft(v => Math.max(100, v - Math.floor(Math.random() * 3))),
      4500
    );
  ...
  const handleJoin = async () => {
    ...
    try {
      ...
      setSnack({
        open: true,
        msg: 'You’re on the waitlist! We’ll email launch updates.',
        severity: 'success',
      });
    } catch {
      setSnack({
  ...
  return (
    <div className='page hero-gradient-bg'>
      ...
      <div className='container'>
        <div className='glass-bar'>
          <div className='bar-left'>
            <Chip className='chip-soft' size='small'>
              <PeopleIcon /> {liveOnline.toLocaleString()} browsing now
            </Chip>
            <Chip className='chip-soft' size='small'>
              <TrendingUpIcon /> {progress}% spots claimed
            </Chip>
          </div>
          <div className='bar-right'>
            <ScheduleIcon fontSize='small' />
            <span className='bar-count'>
              Launch in {countdown.days}d {countdown.hours}h {countdown.minutes}
      ...
      <CreatorsKeepControl onPrimary={() => setModalOpen(true)} />
      ...
      >
        ...
        <DialogContent dividers>
          <p className='muted'>
            Be first in line. We’ll email launch updates only—no spam.
          </p>
          <input
            type='email'
            className='input'
          ...
          />
          {emailError ? (
            <div className='input-help error'>{emailError}</div>
          ) : (
