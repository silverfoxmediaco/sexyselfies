...
const Accordion = ({ children, defaultExpanded = false, className = '', ...props }) => {
  const [expanded, setExpanded] = useState(defaultExpanded);

  return (
    <div className={`accordion ${className}`} {...props}>
      {React.Children.map(children, child => 
        React.cloneElement(child, { expanded, onToggle: () => setExpanded(!expanded) })
      )}
    </div>
  );
};

const AccordionSummary = ({ children, expandIcon, onToggle, expanded, className = '', ...props }) => {
  return (
    <div 
...
const AccordionDetails = ({ children, expanded, className = '', ...props }) => {
  return (
    <div className={`accordion-details ${expanded ? 'expanded' : ''} ${className}`} {...props}>
